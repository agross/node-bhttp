...
prepareDefaults = function(request, response, requestState) {
  ...
  return Promise["try"](function() {
    var _base, _base1, _base2, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8;
    request.responseOptions = {
      discardResponse: (_ref1 = request.options.discardResponse) != null ? _ref1 : false,
      keepRedirectResponses: (_ref2 = request.options.keepRedirectResponses) != null ? _ref2 : false,
      followRedirects: (_ref3 = request.options.followRedirects) != null ? _ref3 : true,
      noDecode: (_ref4 = request.options.noDecode) != null ? _ref4 : false,
      decodeJSON: (_ref5 = request.options.decodeJSON) != null ? _ref5 : false,
      stream: (_ref6 = request.options.stream) != null ? _ref6 : false,
      justPrepare: (_ref7 = request.options.justPrepare) != null ? _ref7 : false,
      redirectLimit: (_ref8 = request.options.redirectLimit) != null ? _ref8 : 10,
...
prepareUrl = function(request, response, requestState) {
  ...
  return Promise["try"](function() {
    ...
    request.options.path = urlUtil.format({
      ...
      query: (_ref1 = request.options.query) != null ? _ref1 : urlOptions.query
    });
...
preparePayload = function(request, response, requestState) {
  ...
  return Promise["try"](function() {
    var containsStreams, fieldName, fieldValue, formDataObject, multipart, streamOptions, valueElement, _i, _len, _ref1, _ref2, _ref3;
    ...
    if ((_ref1 = request.options.method) !== "get" && _ref1 !== "head" && _ref1 !== "delete") {
      if ((request.options.encodeJSON || (request.options.formFields != null)) && !multipart) {
        ...
        if (request.options.encodeJSON) {
          ...
          request.payload = JSON.stringify((_ref2 = request.options.formFields) != null ? _ref2 : null);
        ...
        _ref3 = formFixArray(request.options.formFields);
        for (fieldName in _ref3) {
          fieldValue = _ref3[fieldName];
        ...
        return Promise["try"](function() {
          ...
          if ((request.payloadStream._bhttpStreamWrapper != null) && ((request.payloadStream.options.contentLength != null) || (request.payloadStream.options.knownLength != null))) {
            return Promise.resolve((_ref4 = request.payloadStream.options.contentLength) != null ? _ref4 : request.payloadStream.options.knownLength);
          } else {
...
prepareCleanup = function(request, response, requestState) {
  ...
  return Promise["try"](function() {
    var fixedHeaders, key, value, _i, _len, _ref1, _ref2;
    _ref1 = ["query", "formFields", "files", "encodeJSON", "inputStream", "inputBuffer", "discardResponse", "keepRedirectResponses", "followRedirects", "noDecode", "decodeJSON", "allowChunkedMultipart", "forceMultipart", "onUploadProgress", "onDownloadProgress"];
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      key = _ref1[_i];
      delete request.options[key];
    }
    fixedHeaders = {};
    _ref2 = request.options.headers;
    for (key in _ref2) {
      value = _ref2[key];
  ...
  return Promise["try"](function() {
    ...
    if ((request.cookieJar != null) && (response.headers["set-cookie"] != null)) {
      promises = (function() {
        ...
        _ref1 = response.headers["set-cookie"];
        _results = [];
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          cookieHeader = _ref1[_i];
    ...
    var completedBytes, progressStream, redirects, totalBytes, _ref1, _ref2;
    response.request = request;
    response.requestState = requestState;
    response.redirectHistory = requestState.redirectHistory;
    if (((_ref1 = response.statusCode) === 301 || _ref1 === 302 || _ref1 === 303 || _ref1 === 307) && request.responseOptions.followRedirects) {
      if (requestState.redirectHistory.length >= (request.responseOptions.redirectLimit - 1)) {
        redirects = requestState.redirectHistory.map(function(x) {
      ...
      switch (response.statusCode) {
        ...
        case 307:
          if (request.containsStreams && ((_ref2 = request.options.method) !== "get" && _ref2 !== "head")) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        if (request.responseOptions.stream) {
          ...
          return response.pipe(concatStream(function(body) {
            var err, _ref3;
            if (request.responseOptions.decodeJSON || (((_ref3 = response.headers["content-type"]) != null ? _ref3 : "").split(";")[0] === "application/json" && !request.responseOptions.noDecode)) {
              try {
...
redirectGet = function(request, response, requestState) {
  ...
  return Promise["try"](function() {
    var key, options, _i, _len, _ref1;
    options = _.clone(requestState.originalOptions);
    options.method = "get";
    _ref1 = ["inputBuffer", "inputStream", "files", "formFields"];
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      key = _ref1[_i];
      delete options[key];
    }
  ...
  _doRequest: function(url, options, requestState) {
    return Promise["try"]((function(_this) {
      return function() {
        ...
        if (requestState.sessionOptions == null) {
          requestState.sessionOptions = (_ref1 = _this._sessionOptions) != null ? _ref1 : {};
        }
